...
- → **次: Image Lab の立ち上げ**。研究対象と調査モジュールは [directory.ts](directory.ts) と `/labs/image` に記載済み。進め方: ①ハーネス付き画像生成のディープリサーチ(モデル比較ではなくパイプライン構成の比較)→ ②Motion Lab と同型のカタログUIで実験場を構築(ロゴ切替が全実験に即時反映される構造を踏襲)
- その後: Video Lab(ショートビデオAPI実測=課金設計の数字の根拠づくり)、Workflow Lab(モックアップ自動合成)
