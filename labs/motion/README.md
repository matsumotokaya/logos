...
ロゴのモーション表現・視覚表現の**研究開発ラボ**。プロダクション機能ではなくサンドボックス。`/labs/motion` で開く(noindex、旧 `/lab` はリダイレクト)。研究所群の全体像は [../README.md](../README.md) を参照。
...
app/labs/motion/page.tsx  # 薄いルート(本体への影響はこの1ファイルの追加のみ)
```
