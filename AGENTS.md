...
## ドキュメントとコミットの書き方(2026-08-30 方針)

- 見出しは機能名にする(概要 / 構成 / セットアップ / 未解決の課題 / 制約)
- 結論を先に書き、根拠はファイル名と行番号で示す
- 経緯を本文に書かない。必要なら `docs/old/` か git 履歴に置く
- 比喩・反語・体言止めを使わない
- 変化する値(ファイルサイズ、行数、日数)を指示の根拠にしない
- コミットは Conventional Commits、英語、件名50字以内
