...
| [docs/event-cm-refactor-plan.md](docs/event-cm-refactor-plan.md) | **Event CM リファクタリング計画の正本**(2026-08-15更新): **§9 シナリオ中心への再定義が主題**——「ナレーション」と呼んでいるものは各コマの**字幕＝物語の主文(シナリオ)**であり、それが素材と成果物を規定する。読み上げ音声はその派生物。**§11.1(絵コンテ=作業場・プレイヤー=成果への分離、migration 0050)は実装済み**で、残るは §11.2 `script`→`scenario` 改名と §11.3 `EventCmBrief` の自立(どちらもDB焼き替えを伴う)。あわせて1セッションで出た11件のバグが同じ形であることの分析、**共通原因3つ**、**変えてはいけない契約の一覧(=仕様のSSOT)**、`eventCmFilm()` を中心にした構造、テスト資産と危険箇所。**次にこのテンプレートを触るセッションの出発点** |
...
- **絵コンテは作業場、プレイヤーは成果**([lib/event-cm/bake.ts](lib/event-cm/bake.ts)、migration 0050)。Takeは2つのブリーフを持つ: `brief` は編集がそのまま溜まる作業中の値、`baked_brief` は**実行が固定した映像**で、**プレイヤーもMP4書き出しも公開URLもこちらだけを読む**。だからシナリオを1行直してもプレイヤーの尺と字幕は動かない——**動くのは「動画を作り直す」を押したときだけ**。一致していないのが正常な状態なので、画面はそれを隠さず**プレイヤーの真下で件数を言う**(「まだこの動画へ反映していない変更が2件あります(項目の変更、シナリオの変更)」)。差分を答えるのは `bake.ts` 1箇所で、バーの4段目・ボタンの件数バッジ・プレイヤーの注記はその言い換え。**deep equal はしない**——素材URLは読み込みのたびに署名し直されるので毎回「変更あり」になってしまう。比べるのは `factsUpdatedAt` / `script.updatedAt` / `voice.track.generatedAt` の3つ
  ...
  - **シナリオを直しても音声は消えない**。以前は1行保存するたびに録音を削除していたが、それは「編集は溜まる」に反する(編集の途中で映像が無音に戻る)。古い音声は正常な状態として残り、**差分として言われる**。書き直しは1つのボタンか、ヘッダーの「ナレーション」から
  - **ナレーションのオフは記録される**。オフが「ポインタを外すだけ」だと、次の実行が音声を勝手に戻してしまう(「音声は必ずある」と「オフにできる」が衝突する)。オフは他の非表示項目と同じ suppression として `provenance.voice` に記録し、読み上げを頼んだ時点で解除する
...
- **プレイヤーの真下は絵コンテ**([components/video/Storyboard.tsx](components/video/Storyboard.tsx)、モデルは [lib/storyboard/event-cm.ts](lib/storyboard/event-cm.ts))。1コマ=映像1枚を**1920×1080のアートボードにワイヤーフレームで描き、カード幅に合わせてCSSで縮小**する(slide-factoryのデッキパネルと同じ手法なので、縮小版と拡大版が1実装)。**実物を模写しない**——動く墨の地・金の粒子・タイポの動きは縮小で再現できないので約束しない。忠実なのは絵コンテが読まれる理由の方:どの配置か、何が乗るか、どこに、どの大きさで、何秒、何を読み上げるか。**素材があるものは実画像を描く**(ロゴは映像と同じknockout、登壇者写真は同じ焦点、背景写真は同じ減光)——枠と社名で代用すると、それは代用ではなく**別のデザイン**に見える。**絵コンテが勝手に描く要素にアクセント色(金)を使わない**。この art direction では金は「誰かが決めた」という意味を持つので、ダミーの枠が金だと過去のデザイン案に見える(1コマ目のロゴが実際にそう読まれた)。素材が無いときは映像と同じ設計代替を描く(マークなら明朝のクレジット、登壇者なら金縁モノグラム=これは本物のデザイン)。型の大きさはテーマの型スケール、領域はキットの `REGION_GEOMETRY` から取るので、**テーマや配置を変えると絵コンテが勝手に追随する**
  - **1コマ=1シーン=1メッセージ=ナレーション1行**。だから字幕がカットをまたがない。**プログラムが複数あればその数だけコマになる**——3つのプログラムは3枚で、それぞれが自分の番号(`1 / 3`)と自分の文言と自分のナレーション1行を持つ。番号付きリスト1枚は「1枚に3メッセージ」であり、そのコマのナレーションは「いろいろあります」しか言えない。1つだけなら従来どおり1枚のリスト(既存Takeは何も変わらない)。**シーンの同一性は役割だけでは足りない**ので `role + index`([remotion/event-cm/types.ts](remotion/event-cm/types.ts) の `eventCmSceneKey`)——タイムライン・字幕・絵コンテ・台本の行が「どのコマの話か」で合致する。台本の予算もコマ単位(`eventCmSceneBudget`。1つ目のプログラムだけ全体の前置きを兼ねるので長い)
  - **書きかけの台本は正常な状態**。1行だけ保存でき、言葉の無いコマはそのコマの想定尺に戻り、字幕はそのコマだけ出ない。「全コマ揃っていること」を保存の条件にすると、**コマが増えた直後は1行も保存できなくなる**(3プログラムに割った瞬間、2コマが空なので毎回400で拒否された)。読み上げだけは全コマ必要で、そのときは`3/6コマ`と数で言う
  - **操作の失敗で画面を奪わない**。読み込めない動画は画面ごとエラーにするが、**保存やRunの失敗はインライン帯**で言う(以前は1行保存の失敗で「← 動画一覧へ」のエラーページへ飛び、編集中の文章も失われた)
  - **コマが喋るかどうかは映像の形が決める**(`eventCmNarratedSteps`)。台本にその行があるかどうかでは決めない——構成が変わった直後は「喋るのに行がまだ無いコマ」が正常な状態で、そこに編集欄が出ないと書く場所が無くなる。**行を失ったコマは「使われていない行」として文言ごと画面に残す**(`Storyboard.orphanLines`)。消えたのではなく置き場が無くなっただけなので、そう言う。手で直した台本(`source: "human"`)は自動では書き直されないため、**この警告からだけ`force`付きで書き直せる**(「手で直した行も置き換わります」と明示)
  - **コマの削除はコマの中でする**([lib/event-cm/panel-actions.ts](lib/event-cm/panel-actions.ts))。何が削除できて、できないときは何と言うかの正本はここ(テストは [panel-actions.test.ts](lib/event-cm/panel-actions.test.ts))。**削除できるのは、その不在が映像の取り得る形であるコマだけ**——登壇者のコマ(値は残るので戻せる)と、2つ以上あるプログラムの1つ(プログラム自体が消える)。タイトル・価値・日程・両端のロゴは**削除できず、理由を出す**(「告知は自分の名前を先に言います」など)。空の枠が残るだけの操作を「削除」と呼ばないため。プログラムが1つのときは最後の1つを消せない(枠だけが残るので、そのコマで内容を直す方が正しい)。**壊れる方の削除は2クリック**で、何を消すのかを名前で言う。**削除するとモーダルは閉じ、結果はページ側の`role="status"`で言う**——消えた面が自分の結果を報告することはできないため(パネルはブリーフが返った瞬間にアンマウントする)。**コマは位置ではなく「どのコマか」でReactのkeyを持つ**: 6コマ目を消すと7コマ目が6番になり、同じkeyだとReactが同じインスタンスを再利用して、開いたままのモーダルが別のコマを表示し、ナレーションの下書きが次のコマへ持ち越された(保存すれば上書きされる状態だった)
  - **登壇者のコマは、登壇者を「消す」と消える**。プログラム側で人を語る構成のときはこれで足りる(独自ルールを足していない)。**消した項目は読み上げもしない**——APIは台本も音声も`applySuppression`後のブリーフで数える。ブリーフ自体は消さずに保存する(非表示は決定であって削除ではない)プレゼンが1つのメッセージを2枚のスライドに置かないのと同じで、これは製品CM・サービス動画にも共通する原則
  - **コマを開くとその中身を直せる**。**ナレーションもそのコマで直す**——モーダルの本文が編集欄で、1行ずつ保存できる。保存すると台本は`human`になり(再実行で上書きされない)、**音声は外れる**(古い言葉を読み続ける声は無音より悪い)。だから作業の形は「何行か直す → ヘッダーの『ナレーション』でまとめて読み上げ直す」になる。モーダルは見出しを持たない——編集欄と一覧と直す場所が並んでいれば、それが何かは見れば分かる。モーダルは同じアートボードを大きく描き、下に**そのコマが映しているフィールドだけ**の `FactList` を出す。「この日付は仮です」と読む場所と直す場所を分けない。部品→briefフィールドの対応は `SceneComponent.fields` がビルダーで記録する(レンダラーは無視する)
- **構成は固定・ロゴで始まりロゴで終わる**([remotion/event-cm/types.ts](remotion/event-cm/types.ts) の `EVENT_CM_SCENES` が正本): `logoIn → title → value → program → guests → cta → logoOut`。両端の2枚は**無音**で、提供する会社のマークだけを見せる(**それぞれ4秒**。1秒台だとマークが瞬間的に現れて消え、クレジットではなく不具合に見える)。冒頭は音楽のみ、末尾はフェード。**マークは既定で白抜き**(`treatment: "knockout"` = `brightness(0) invert(1)`)——墨の地に黒いSVGを素で置くと見えないため。`invert` は元が明るいマークを壊すので既定にしない。**誰の動画なのか**を最初と最後に言うため。ナレーションは2枚目の**タイトルコールから始まる**——告知は自分の名前を先に言う。`guests` は登壇者が居なければ**シーンごと消える**(居ない人の紹介をしない)ので、台本は4行または5行になる。`eventCmScenePlan()` が唯一の判定で、タイムライン・字幕・絵コンテ・台本プロンプトが全部これを読む
- **ナレーションがオントロジー**: 台本の役割がシーンの並びを決め、尺は「各シーンの想定尺 → 台本の文字数 → 読み上げの実測」と段階的に精度が上がる([remotion/event-cm/timeline.ts](remotion/event-cm/timeline.ts))。だから**動画を追加した瞬間に、LLMもレンダリングも無しで完成した映像が再生される**
...
- **BGMもナレーションと同じ形**([components/video/BgmDialog.tsx](components/video/BgmDialog.tsx))。ヘッダーの「BGM」ボタン(ナレーションの左)にドットで在/不在が出て、モーダルで既定プール+アップロード音声から選ぶ。絵コンテ下のBGM欄は撤去——同じ種類の問い(常時オン/オフ・1つ選ぶ)に答える操作が、片方はヘッダーのボタンで片方はピルの列という状態を作らない。**書き出しに乗らない仮素材はその場で明示**する
- **音楽や写真を選び直しても台本は古くならない**([lib/event-cm/facts.ts](lib/event-cm/facts.ts) の `isSpokenFact`)。`factsUpdatedAt` はナレーションが古いことを言うためだけに在るので、**読み上げないもの**(BGM・写真・ロゴ)では動かさない。何も壊れていないのに警告が出る状態は、警告を無視する習慣を作る
- **ナレーションは1つのボタンで、いつでも上書きできる**([components/video/NarrationDialog.tsx](components/video/NarrationDialog.tsx))。ラベルは常に「ナレーション」で、状態はドットだけが示す——読み上げは完了する工程ではなく、言葉も声も読み方も作り直す対象だから。モーダルで**声を選べる**(標準/男性1/男性2/女性1/女性2 = [lib/narration/voices.ts](lib/narration/voices.ts)。**男女のラベルはこちらの判断**で、Geminiのカタログは性別を明記していないため、Gemini名と性格語を画面に併記して耳で正せるようにしている)。**オフにもできる**——音楽と字幕だけの動画は未完成ではない。オフは`brief.voice`のポインタだけを外し、録音はR2とTakeに残るので戻せる。オフのとき尺は台本推定へ、字幕は台本へ、BGMは全編フルに戻る(いずれも読み上げ前から在る挙動)
...
- **字幕は必須**([remotion/event-cm/captions.ts](remotion/event-cm/captions.ts))。ミュートで見られる前提なので、**音声を待たず台本から**文単位で出し、読み上げ後に実測へ置き換わる。**1枚は28字まで**——句点で割るのが基本だが、句点の無い長い一文は読点で、それも無ければ字数で割る(字幕帯に行数制限は無いので、1200字の「一文」は絵を覆う板になる)
- **台本の長さはそのままコマの長さ**([remotion/event-cm/timeline.ts](remotion/event-cm/timeline.ts) の `writtenMs` = `max(2.5秒, 文字数÷7)` + 間0.9秒)。**上限は無い**——2900字を書けばそのコマは約415秒になり、動画は7分を超える。それが正しい設計(尺は台本が決める)なので止めないが、**書いている場所で見えるようにする**: 編集欄に`目安〜字・約N秒`を出し、目安超過は琥珀色、**読み上げ上限2000字を超えたら赤で「読み上げられません」**([lib/narration/limits.ts](lib/narration/limits.ts))。読み上げは**1リクエストも投げる前に**長さを検査して日本語で断る(以前は3行目で providerの英語エラーになり、1〜2行目の課金だけが発生していた)
- **事実が変わると台本が古くなる**。`brief.factsUpdatedAt` と `script.updatedAt` の比較で判定し、マッピング段が自動で書き直す。ただし `script.source === 'human'` は上書きしない
- **BGMはシステムが持つ**([lib/assets/defaults.ts](lib/assets/defaults.ts))。既定は固定で、冒頭のロゴシーンのあいだ単独で鳴り、ナレーション区間で下がって締めのロゴで戻る。`licensed: false` の素材は書き出しに乗せない
...
スキーマの正本は [supabase/migrations/](supabase/migrations/) の連番migration。現行のリモートプロジェクトには`0050_take_baked_brief`まで適用済み。`0048`はナレーション音声をどの動画テンプレートからでも固定できるようRPCを一般化し、`0049`は`take_runs.stage`に`map`を足してマッピング段が自分の実行を記録できるようにした。`0050`は`takes`に`baked_brief` / `baked_at`を足し、**編集が溜まる作業中のブリーフと、実行が固定した映像を分けた**(上記「絵コンテは作業場、プレイヤーは成果」。既存 event-cm Take は `baked_brief = brief` でバックフィル済みなので、適用しても今日の見え方は変わらない)。`0023`〜`0045`でV2基盤・データ移行・旧契約削除・保全データ修復を完了し、`0046`で既存Brandへのロゴ追加とcanonicalロゴプレゼンTake生成を原子的に統一、`0047`で既存 event-promo Take からブリーフ+`take_inputs` を持ち運んで新規Takeを作るRPC `clone_event_promo_take` を追加した。新規URL生成は旧Profile/Generation Run/Assetへ二重書きせず、Knowledge claims + Take Run + Takeを正本にする。移行記録は [docs/schema-v2.md](docs/schema-v2.md)、現在の契約は [docs/data-model.md](docs/data-model.md)。新規環境のセットアップ手順:

1. Supabase の SQL Editor で `supabase/migrations/` 内のSQLを番号順に実行(0001→0050)
