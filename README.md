...
- **素材の正規化とインベントリ**: 正本は [docs/asset-normalization.md](docs/asset-normalization.md)(2026-08-16 第2版)。素材の正規化を**サービスの第一価値**と位置づけ、**2層の注入モデル**(ブランドの基盤 → 成果物ごとの上書き・オフ)と**インベントリUI**(絵コンテ下の2段セクション)を設計した。**段階1「測って残す」は実装済み・migration 0052 適用済み**——測定の正本は [lib/materials/measure.ts](lib/materials/measure.ts) で、取り込み3経路が `width`/`height`/`opaque`/`luminance` を行に書く。**残っているのは既存47行のバックフィル** `npm run materials:measure --apply`(要ユーザー承認。dry-runは実施済みで36件が測定可能・読めなかったもの0件)。**次は §14 の段階2**(`category` の語彙を決めて構造化LLMの判定を行へ書く)、段階4(インベントリUIの読み)までは既存データだけで動く
