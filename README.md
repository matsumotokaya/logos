...
| [docs/event-cm-refactor-plan.md](docs/event-cm-refactor-plan.md) | **Event CM リファクタリング計画の正本**(2026-08-15更新): **§9 シナリオ中心への再定義が主題**——各コマの**字幕＝物語の主文(シナリオ)**が素材と成果物を規定し、読み上げ音声はその派生物。**§11.1(絵コンテ=作業場・プレイヤー=成果、migration 0050)と §11.2(`script`→`scenario` 改名+UI語彙の3分割、migration 0051)はどちらも実装済み**。残るは **§11.3 `EventCmBrief` の自立**(`extends EventBrief` をやめる。zodがstripなら焼き替え不要)と §9.6 の宿題(ドロワー内の個別実行ボタン)。あわせて1セッションで出た11件のバグが同じ形であることの分析、**共通原因3つ**、**変えてはいけない契約の一覧(=仕様のSSOT)**、`eventCmFilm()` を中心にした構造、テスト資産と危険箇所。**次にこのテンプレートを触るセッションの出発点** |
...
| `/brands/[id]/video/[videoId]` | Brand配下の動画詳細。末尾はTake ID。プレイヤーの直下は**絵コンテ**(コマ=映像1枚のワイヤーフレーム。クリックで拡大し、そのコマの項目をその場で直す)。ページ上部の**4段パイプライン(入力・抽出/構造化/マッピング/出力)**は**ドロワーで開いて中で作業する**——資料の投入、読み取り、構造化、動画への反映、事実の修正がすべて段の中で完結する。各ドロワーは**自分の実行**と**次の段へ進むボタン**を持ち、進むボタンは**次のドロワーを開いて同時にその段を実行する**(slide-factoryの`navigate()`→`start...()`と同じ順序)。読むことは反映することではないので、成功して終わりの行き止まりを作らない。一括実行(**「動画を作り直す」**)はバーの外。**未処理があればバッジで件数を出し、無ければグレーになるが押せる**——押すと最初からもう一度全部走る。段の中身はLLM呼び出しなので、「全部済んでいる」と「もう一度頼みたいことが無い」は別のことだから(資料が1件も無いときだけ無効)。**このボタンはMP4を含まない**(下記「絵コンテは作業場、プレイヤーは成果」)。ドロワー内の**段ごとのボタンは、やることが無ければ無効になる**(「すべて読み取り済みです」と理由を出す)。**時間のかかる処理は必ず画面上端にバーと文言が出る**([components/pipeline/BusyBar.tsx](components/pipeline/BusyBar.tsx))——資料の登録、ナレーションの執筆・読み上げ、MP4の書き出し、事実の修正まで全部。**割合は出さない**(LLM呼び出しに正直な進捗率は無く、90%で止まる数字は無いより悪い)。段の実行はさらに右下のカードで何が見つかったかが見え、`take_runs` に永続する。**実行ログは要約しない**: 行を開くと入力・出力・使用量・エラー・所要時間がそのまま出て、1件ずつでも全件でもコピーできる([components/pipeline/TakeRunLog.tsx](components/pipeline/TakeRunLog.tsx))。イベント紹介動画(`event-cm`)はシナリオ駆動で、シナリオ・字幕・尺・読み上げが連動する。Product CMでvoiceが未固定のときは「CM Makerで再生成する」ボタンが出る |
