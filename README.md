...
- **素材の正規化とインベントリ**: 正本は [docs/asset-normalization.md](docs/asset-normalization.md)(2026-08-16 第2版)。素材の正規化を**サービスの第一価値**と位置づけ、**2層の注入モデル**(ブランドの基盤 → 成果物ごとの上書き・オフ)と**インベントリUI**(絵コンテ下の2段セクション)を設計した。**段階1「測って残す」と段階2「軸1を列にする」は実装済み**(migration 0052 / 0053 適用済み)。測定の正本は [lib/materials/measure.ts](lib/materials/measure.ts)、**分類の語彙の正本は [lib/materials/category.ts](lib/materials/category.ts)**。**ファイルはサービス共通の絶対軸1つで分類し、情報はテンプレートごとのオントロジーが持つ**(§5が正本)——判定は「開けるものか、読むものか」で、価格・会場名・お客様の声は読むものなのでbriefの側。だから語彙は10値のまま増えにくい(新テンプレートが持ち込むのはたいてい新しい**情報**で、ファイルの軸を揺らさない)。構造化LLMは**2つを別々に答える**(`role`=この動画のどのスロットか / `category`=何が写っているか)。**残っているのは (a) 既存47行のバックフィル** `npm run materials:measure --apply`(dry-run実施済み・36件測定可能・読めなかったもの0件)、**(b) 分類を直すプルダウン**(`category_source='user'`を立てる操作面。**インベントリUI=段階4と同時に作る**)。**次は §14 の段階3(命名)または段階4(インベントリUI)**
