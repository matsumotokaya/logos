...
- **不正な `treatment` を持つ既存Take が1件ある**: `920d4cfe-3843-4a6c-b538-7238e79707d2`(世界が恋する日本酒)の `brief` / `baked_brief` 両方の `logos[0].treatment` が `knockout` のまま。**規則は直したがデータは直っていない**ので、この動画のロゴはいまも真っ白の板で描かれる。直し方は2つ——(a) マッピング段をもう一度実行する(`placeImagesIntoBrief` が自己修復して `light` へ直し、実行ログに理由が残る。ただし`baked_brief`は「動画を作り直す」まで古いまま)、(b) SQLで両方の列を直接直す(**要ユーザー承認**)。(a) が筋
...
- **実ブラウザでの通し確認が未実施**: 2026-08-15 に触った画面(絵コンテのマッピング表示、ステータス行、ヘッダーのドット、入力ドロワーのボタン順)は型・テスト・ビルドは通っているが、**目視していない**
