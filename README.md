...
| [docs/event-cm-refactor-plan.md](docs/event-cm-refactor-plan.md) | **Event CM リファクタリング計画の正本**(2026-08-15更新): **§9 シナリオ中心への再定義が主題**——各コマの**字幕＝物語の主文(シナリオ)**が素材と成果物を規定し、読み上げ音声はその派生物。**§11 のパケットは全部完了**: §11.1(絵コンテ=作業場・プレイヤー=成果、migration 0050)、§11.2(`script`→`scenario` 改名+UI語彙の3分割、migration 0051)、§11.3(`EventCmBrief` の自立=`extends EventBrief` の廃止。zodがstripするのでDB焼き替え不要)、§11.3b(§9.6 の残り=マッピング段から映像段へ進むボタン、判定は `lib/pipeline/stage-actions.ts`)。**§10 に以後従う規則を5つ列挙**。残るのは §11.4 の小さな穴と §8 の負債のみ。あわせて1セッションで出た11件のバグが同じ形であることの分析、**共通原因3つ**、**変えてはいけない契約の一覧(=仕様のSSOT)**、`eventCmFilm()` を中心にした構造、テスト資産と危険箇所。**次にこのテンプレートを触るセッションの出発点** |
| [docs/event-cm-image-screening-plan.md](docs/event-cm-image-screening-plan.md) | **Event CM 画像の自動選定と背景写真の計画**(2026-08-14 全面改訂): 画像の意味判定を構造化の1回に統合する方針、決定論の配置規則、由来4値だけを台帳にする理由、`event-cm` へ背景写真(`Scene.backdrop`)を持たせるテンプレート変更、日本酒の映画から移す技術、実装順序と受入条件、slide-factory から借りないもの |
...
- **ブリーフは event-promo とフィールドを共有しない**(`EventCmBrief`。[remotion/event-cm/types.ts](remotion/event-cm/types.ts))。共有するのは**値の型**(写真・ロゴ・登壇者・プログラム・日程)だけで、**フィールドの一覧は別**。以前は `extends EventBrief` だったため、`sideCopy` / `visuals.inkArt` / `visuals.texture` という**どのシーンも描かない3つ**を持っていて、それが項目一覧に並んでいた——埋めても画面は何も変わらない。**ブリーフに足したフィールドは、どこかのシーンが読むこと**が以後の規則。地(背景写真)を受け取るのは `visuals.value` / `visuals.programs` / `visuals.closing` の3枚だけ
  ...
  以前はこの3つを全部「ナレーション」と呼んでいた。名前が逆向きだったのが原因で、字幕は音声から作られるのか主文から作られるのかが読む場所によって違って見え、実装は正しいのに説明だけが逆という状態が続いた(経緯は [docs/event-cm-refactor-plan.md §9.1](docs/event-cm-refactor-plan.md))。**product-cm はまだ `cm_script` のまま**で、共通語彙にするかは別判断。共有コードの [lib/narration/voice.ts](lib/narration/voice.ts) はどちらの綴りも知らない(渡された言葉を読むだけ)

**「このブリーフはどんな映像になるか」の唯一の導出は [remotion/event-cm/film.ts](remotion/event-cm/film.ts) の `eventCmFilm()`**。レンダラー(`EventCmComposition`)・絵コンテ(`eventCmStoryboard`)・シナリオ/音声APIはすべてこの戻り値を読み、suppression・尺・部品・字幕を自分で導出しない(消した項目を空にする `applySuppression` は film の非公開ステップで、他から呼べない)。この構造にした経緯・変えてはいけない契約・残りの作業は [docs/event-cm-refactor-plan.md](docs/event-cm-refactor-plan.md) が正本。
