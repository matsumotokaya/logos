...
| [docs/asset-normalization.md](docs/asset-normalization.md) | **素材の正規化とインベントリの正本**(2026-08-16 仕様・実装前): 雑に入れた素材が整理されて出てくるための、2軸モデル(素材の**種別** / この成果物での**役割**)、ロゴの下位属性(地の有無・明度・主体・解像度)、**ファイル名ドリブン**の命名と「R2キーは内容アドレス・名前は行が持つ」、書き出しに同じ整理が出ること、余白トリミングを**派生素材**として作ること、未決定6件と4段階の進め方。**次に素材まわりを触るセッションの出発点** |
...
- **素材の正規化とインベントリ**: 仕様を [docs/asset-normalization.md](docs/asset-normalization.md) に書いた(2026-08-16・**実装はこれから**)。2軸モデル(素材そのものの種別 / この成果物での役割)、ロゴの下位属性、ファイル名ドリブンの命名、書き出しに同じ整理が出ること、余白トリミングを派生素材として作ること。**着手はその文書の §12「段階」の1番目から**——`width`/`height`/`opaque`/`luminance`/`source_url` を取り込み時に埋めるだけの migration で、見た目は変わらないのに白板バグの再発条件が消える
