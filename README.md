...
| [docs/asset-normalization.md](docs/asset-normalization.md) | **素材の正規化とインベントリの正本**(2026-08-17・**段階1〜6実装済み**): 素材の正規化を**サービスの第一価値**として位置づけ。**§5 ファイルと情報の2軸**(ファイルはサービス共通の絶対軸1つ・情報はテンプレートごとのオントロジー。判定は「開けるものか、読むものか」)、**§7.0 基盤はLP・バナー・動画すべての手前にある層**(置けるのは「この会社について真であること」だけ。判定は「その値はLPでも意味を持つか」)、**既定値の4層**(ブランド→テンプレート→システム→設計代替)、ロゴの下位属性と測定、**§8 名前は保存せず導出する・意味は発明しない**、インベントリUIの2段、昇格と基盤のオフ、余白トリミング。**次に素材まわりを触るセッションの出発点** |
...
| [remotion/kit/theme.ts](remotion/kit/theme.ts) | パレット/書体/4段の型スケール/**モーション語彙**/装飾/字幕。`SUMI_THEME` と `themeForBrand()`。**字幕帯は聖域**で、`captionSafeBottom()` が下寄せ領域の下限を決める(以前は訴求シーンのコピーが字幕を貫いていた) |
