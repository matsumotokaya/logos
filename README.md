...
スキーマの正本は [supabase/migrations/](supabase/migrations/) の連番migration。現行のリモートプロジェクトには`0031_deletion_rpcs`まで適用済み。`0023`〜`0031`がv2アーキテクチャの土台で、URL生成のLP writerと`/c/[id]`のPublication readerはv2へ接続済み、Brand管理画面と既存動画ポータルはv1のまま並行稼働している。設計と切替状況は [docs/schema-v2.md](docs/schema-v2.md)。新規環境のセットアップ手順:
