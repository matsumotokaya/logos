...
- **素材の分類を素材側に持たせる(2軸モデル)**: いま画像の分類(`speaker-portrait` / `key-visual` / `logo` / `document` / `texture` / `unreadable` …)は構造化LLMが**毎回その場で判定**し、`take_runs.steps` の実行記録にしか残らない。`brand_materials` に列が無いため、(1) 再実行のたびに再判定・再課金、(2) 素材一覧を分類で絞れない、(3) 別Takeへ引き継げない。**依頼者の提案は2軸**——「**素材そのものの分類**(人物・製品・ロゴ・背景・装飾・資料)」と「**この動画における役割/スロット**(キービジュアル・プログラムの地・登壇者の写真)」を分け、どちらもプルダウンで**手で直せる**ようにする。キービジュアルが人物にも製品にも付き得るのは、割り当てがイベントのオントロジー次第だから。**語彙は現在 event-cm 専用**(`speaker-portrait` / `venue` があり `product` が無い)なので、テンプレート非依存の語へ作り直す必要がある。migration を伴うので、着手前に仕様を1枚書く
- **event-cm のMP4書き出しが繋がっていない**(2026-08-16 発見): `EventCmComposition` は `remotion/Root.tsx` に登録されておらず、`lib/takes/render.ts` の `produce()` にも分岐が無い(`event-promo` / `product-cm` / `campaign-lp` のみ)。つまりヘッダーの「MP4を作成」は event-cm では通らない。プロジェクト書き出しは自前のエントリを生成するので独立して動く(そちらで手元レンダーはできる)。直すときは、書き出しが生成する `src/Root.tsx` と同じ登録を `remotion/Root.tsx` に足し、`produce()` に arm を1本足す
