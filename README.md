...
| 語 | 実体 | コード |
| --- | --- | --- |
| **シナリオ** | 各コマの**主文**。字幕になり、素材と成果物と尺を規定する**主**。「この動画は何を言うか」 | `brief.scenario`(`EventCmScenario`。旧 `brief.script`) |
...
- **絵コンテがマッピングそのもの**。各コマの下に、**そのコマが映しているフィールド**が値と一緒に並ぶ(`PanelFacts`)。開催日・開始時刻は締めのコマ、登壇者の写真は登壇者のコマ、**ロゴは冒頭・締め・末尾の3コマに重複して出る**(実際に3回画面に出るため)。以前は盤面の下に**ブリーフ全体の平らな一覧**「この動画は何でできているか」があったが、それは「何でできているか」には答えても、絵コンテを読む人が知りたい**「どこに載るか」**を落としていた。全体の一覧はパイプラインの**マッピング段のドロワー**に残る(全部を見る場所はそこ)。**盤面の一覧は読み取り専用**で、直すのは従来どおりコマの中(2つの編集面を作らない)。どのコマにも載らないもの(BGM・読み上げ・動画名)はヘッダーのボタンが持っているので、「基本情報」セクションは作らない——同じ2つの事実を読む場所を3箇所にしないため。**空の任意項目は盤面に出ない**(映像が描かないので)。それはドロワーの「不足」に並ぶ
- **プレイヤーの真下は絵コンテ**([components/video/Storyboard.tsx](components/video/Storyboard.tsx)、モデルは [lib/storyboard/event-cm.ts](lib/storyboard/event-cm.ts))。1コマ=映像1枚を**1920×1080のアートボードにワイヤーフレームで描き、カード幅に合わせてCSSで縮小**する(slide-factoryのデッキパネルと同じ手法なので、縮小版と拡大版が1実装)。**実物を模写しない**——動く墨の地・金の粒子・タイポの動きは縮小で再現できないので約束しない。忠実なのは絵コンテが読まれる理由の方:どの配置か、何が乗るか、どこに、どの大きさで、何秒、何を読み上げるか。**素材があるものは実画像を描く**(ロゴは映像と同じknockout、登壇者写真は同じ焦点、背景写真は同じ減光)——枠と社名で代用すると、それは代用ではなく**別のデザイン**に見える。**絵コンテが勝手に描く要素にアクセント色(金)を使わない**。この art direction では金は「誰かが決めた」という意味を持つので、ダミーの枠が金だと過去のデザイン案に見える(1コマ目のロゴが実際にそう読まれた)。素材が無いときは映像と同じ設計代替を描く(マークなら明朝のクレジット、登壇者なら金縁モノグラム=これは本物のデザイン)。型の大きさはテーマの型スケール、領域はキットの `REGION_GEOMETRY` から取るので、**テーマや配置を変えると絵コンテが勝手に追随する**
  - **1コマ=1シーン=1メッセージ=シナリオ1行**。だから字幕がカットをまたがない。**プログラムが複数あればその数だけコマになる**——3つのプログラムは3枚で、それぞれが自分の番号(`1 / 3`)と自分の文言と自分のシナリオ1行を持つ。番号付きリスト1枚は「1枚に3メッセージ」であり、そのコマのシナリオは「いろいろあります」しか言えない。1つだけなら従来どおり1枚のリスト(既存Takeは何も変わらない)。**シーンの同一性は役割だけでは足りない**ので `role + index`([remotion/event-cm/types.ts](remotion/event-cm/types.ts) の `eventCmSceneKey`)——タイムライン・字幕・絵コンテ・シナリオの行が「どのコマの話か」で合致する。シナリオの予算もコマ単位(`eventCmSceneBudget`。1つ目のプログラムだけ全体の前置きを兼ねるので長い)
  - **書きかけのシナリオは正常な状態**。1行だけ保存でき、言葉の無いコマはそのコマの想定尺に戻り、字幕はそのコマだけ出ない。「全コマ揃っていること」を保存の条件にすると、**コマが増えた直後は1行も保存できなくなる**(3プログラムに割った瞬間、2コマが空なので毎回400で拒否された)。読み上げだけは全コマ必要で、そのときは`3/6コマ`と数で言う
  - **操作の失敗で画面を奪わない**。読み込めない動画は画面ごとエラーにするが、**保存やRunの失敗はインライン帯**で言う(以前は1行保存の失敗で「← 動画一覧へ」のエラーページへ飛び、編集中の文章も失われた)
  - **コマが喋るかどうかは映像の形が決める**(`eventCmNarratedSteps`)。シナリオにその行があるかどうかでは決めない——構成が変わった直後は「喋るのに行がまだ無いコマ」が正常な状態で、そこに編集欄が出ないと書く場所が無くなる。**行を失ったコマは「使われていない行」として文言ごと画面に残す**(`Storyboard.orphanLines`)。消えたのではなく置き場が無くなっただけなので、そう言う。手で直したシナリオ(`source: "human"`)は自動では書き直されないため、**この警告からだけ`force`付きで書き直せる**(「手で直した行も置き換わります」と明示)
  - **コマの削除はコマの中でする**([lib/event-cm/panel-actions.ts](lib/event-cm/panel-actions.ts))。何が削除できて、できないときは何と言うかの正本はここ(テストは [panel-actions.test.ts](lib/event-cm/panel-actions.test.ts))。**削除が何を意味するかもテンプレートが決める**——画面は「このコマを消して」と頼むだけで、`DELETE /api/brands/[id]/videos/[videoId]/panels` が同じ関数を引いてから書く(登壇者は suppression、プログラムは一覧から1つ落とす)。組み立てを画面側に置くと、ボタンがブリーフの2人目の著者になる。断りは409で、テンプレート自身の日本語がそのまま絵コンテ上の帯に出る。**削除できるのは、その不在が映像の取り得る形であるコマだけ**——登壇者のコマ(値は残るので戻せる)と、2つ以上あるプログラムの1つ(プログラム自体が消える)。タイトル・価値・日程・両端のロゴは**削除できず、理由を出す**(「告知は自分の名前を先に言います」など)。空の枠が残るだけの操作を「削除」と呼ばないため。プログラムが1つのときは最後の1つを消せない(枠だけが残るので、そのコマで内容を直す方が正しい)。**壊れる方の削除は2クリック**で、何を消すのかを名前で言う。**削除するとモーダルは閉じ、結果はページ側の`role="status"`で言う**——消えた面が自分の結果を報告することはできないため(パネルはブリーフが返った瞬間にアンマウントする)。**コマは位置ではなく「どのコマか」でReactのkeyを持つ**: 6コマ目を消すと7コマ目が6番になり、同じkeyだとReactが同じインスタンスを再利用して、開いたままのモーダルが別のコマを表示し、シナリオの下書きが次のコマへ持ち越された(保存すれば上書きされる状態だった)
  - **登壇者のコマは、登壇者を「消す」と消える**。プログラム側で人を語る構成のときはこれで足りる(独自ルールを足していない)。**消した項目は読み上げもしない**——APIはシナリオも音声も`applySuppression`後のブリーフで数える。だから**消すことは事実の変更**で、`factsUpdatedAt` が動く(動かさないと、シナリオは消えた登壇者の話をしたまま「最新」を名乗り、`bakeState` は差分ゼロと答え、「動画を作り直す」が何もしない)。ブリーフ自体は消さずに保存する(非表示は決定であって削除ではない)プレゼンが1つのメッセージを2枚のスライドに置かないのと同じで、これは製品CM・サービス動画にも共通する原則
  - **コマを開くとその中身を直せる**。**シナリオもそのコマで直す**——モーダルの本文が編集欄で、1行ずつ保存できる。保存するとシナリオは`human`になり(再実行で上書きされない)、**音声も再生中の映像もその場では動かない**(上記「編集は溜まる」)。だから編集欄はそう言う——「保存しても再生中の動画は変わりません。直し終えたら『動画を作り直す』を押してください」。作業の形は「何行か直す → 1つのボタンで通す」で、声だけ差し替えたいときはヘッダーの『読み上げ』から。モーダルは見出しを持たない——編集欄と一覧と直す場所が並んでいれば、それが何かは見れば分かる。モーダルは同じアートボードを大きく描き、下に**そのコマが映しているフィールドだけ**の `FactList` を出す。「この日付は仮です」と読む場所と直す場所を分けない。部品→briefフィールドの対応は `SceneComponent.fields` がビルダーで記録する(レンダラーは無視する)
...
- **章のあいだに間がある**(`EVENT_CM_SCENE_GAP_MS` = 0.9秒)。1コマ=1章なので、間が無いと前の行が着地する前に次が始まる。音楽は止めず、声だけが待つ。**同じ値をTTSのセクション間ギャップにも渡す**ので、録る前の推定と実測が同じリズムを指す。結果として尺は30秒を超える(素の状態で約34秒、登壇者ありで約39秒)——**30秒は要件ではなく、正しい尺が要件**
...
- **シナリオの長さはそのままコマの長さ**([remotion/event-cm/timeline.ts](remotion/event-cm/timeline.ts) の `writtenMs` = `max(2.5秒, 文字数÷7)` + 間0.9秒)。**上限は無い**——2900字を書けばそのコマは約415秒になり、動画は7分を超える。それが正しい設計(尺はシナリオが決める)なので止めないが、**書いている場所で見えるようにする**: 編集欄に`目安〜字・約N秒`を出し、目安超過は琥珀色、**読み上げ上限2000字を超えたら赤で「読み上げられません」**([lib/narration/limits.ts](lib/narration/limits.ts))。読み上げは**1リクエストも投げる前に**長さを検査して日本語で断る(以前は3行目で providerの英語エラーになり、1〜2行目の課金だけが発生していた)
...
- **[docs/video-state-model.md](docs/video-state-model.md) の残り**: §5 のうち **FactListの「未反映」チップ**と**絵コンテのコマの琥珀の点**が未実装
...
- **絵コンテのコマ1・コマ9が、描いていないロゴまで名乗る**: `markScene` は「提供者のマーク」1つ(`logos[0]`)だけを描くのに `fields: ["logos"]` を宣言するため、`FactList` が配列全体(「WealthPark Lab、leopalace21」)を並べる。絵は1つ、値は2つ。コマ8(`logoRow`)は実際に両方描くので正しい。`FactList` は固定パス台帳(`factFieldsFor`)で解決していて `logos.0` のような添字パスを持てないため、直すには**台帳側に添字を入れるか、部品が描いた値を `FactList` へ渡すか**の設計判断が要る。絵コンテは「どこに載るか」に答える画面なので、放置すると約束を外し続ける
