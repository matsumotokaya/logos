...
- **Remotionプロジェクトファイルの配布**(相談段階・未着手): `baked_brief` を `inputProps` として、素材・ナレーションWAV・タイミングJSONと一緒にzipで配る案。**必要な部品はすべて既にある**(MP4レンダー時の素材ステージングと同じ経路)。`render_artifacts` に `format: "zip"` として置けば公開・履歴・署名配信がそのまま使える。Remotion本体は再配布不可なので `package.json` の依存として書き、受け取った人が自分のライセンスで入れる。`licensed: false` の素材は同梱しない(**ただしその除外は未実装**・下記)
- **`licensed: false` の書き出し除外が未実装**: `unlicensedDefaults()` に呼び出し元が無い。既定BGM 2曲はSuno商用可プランなので現状リスクはないが、清算されていない曲を足すなら先にレンダー側の除外を書く
