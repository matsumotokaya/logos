...
**このセッションでやったこと**: 素材の正規化を段階1〜6まで実装した(正本は [docs/asset-normalization.md](docs/asset-normalization.md))。migration `0052`(測定)・`0053`(分類)を適用し、既存47行のバックフィルも実行済み。

**まず読むもの**: [docs/asset-normalization.md](docs/asset-normalization.md) の §5(ファイルと情報の2軸)と §7.0(基盤はすべての営業アセットの手前にある)。この2つがこのセッションで確定した設計判断で、以降の迷いはほぼここに答えがある。
...
- **既定値は4層**: ブランド → テンプレート(`defaultBgm` / `defaultVisuals`)→ システム(`DEFAULT_NARRATION_VOICE`)→ 設計代替(墨の地)。**4層目があるから上は空でよい**
